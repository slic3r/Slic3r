...
sub options {
    return qw(
        ...
        top_solid_layers min_shell_thickness min_vertical_shell_thickness bottom_solid_layers
...
sub build {
    ...
    
    {
        ...
        }
        {
            ...
            $optgroup->append_single_option_line('min_vertical_shell_thickness');
        }
        {
...
sub _update {
    ...
    if (any { /$opt_key/ } qw(all_keys spiral_vase perimeters top_solid_layers fill_density support_material min_shell_thickness min_vertical_shell_thickness)) {
        if ($config->spiral_vase && !($config->perimeters == 1 && $config->min_shell_thickness == 0 && $config->min_vertical_shell_thickness == 0 && $config->top_solid_layers == 0 && $config->fill_density == 0 && $config->support_material == 0)) {
            ...
            if ($dialog->ShowModal() == wxID_YES) {
                ...
                $new_conf->set("min_vertical_shell_thickness", 0);
    ...
    my $have_solid_infill = ($config->top_solid_layers > 0) || ($config->bottom_solid_layers > 0) || ($config->min_vertical_shell_thickness > 1e-4);
    ...
    my $have_top_solid_infill = ($config->top_solid_layers > 0) || ($config->min_vertical_shell_thickness > 0);
