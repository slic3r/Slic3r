...
sub write_gcode {
	...
	#Only print it the internal gcode setup hasn't been disable
	if (!$Slic3r::Config->overide_internal_start_end_gcode) {
	...
	if ($Slic3r::Config->start_gcode ne "") {
	...
	#Only print it the internal gcode setup hasn't been disable
	if (!$Slic3r::Config->overide_internal_start_end_gcode) {
		printf $fh "G90%s\n", ($Slic3r::Config->gcode_comments ? ' ; use absolute coordinates' : '');
		printf $fh "G21%s\n", ($Slic3r::Config->gcode_comments ? ' ; set units to millimeters' : '');
	}
		...
		if ($Slic3r::Config->gcode_flavor =~ /^(?:reprap|teacup)$/) {
			...
			if ($Slic3r::Config->gcode_flavor =~ /^(?:reprap|makerbot|replicator2)$/) {
				if ($Slic3r::Config->use_relative_e_distances) {
					printf $fh "M83%s\n", ($Slic3r::Config->gcode_comments ? ' ; use relative distances for extrusion' : '');
				} else {
					printf $fh "M82%s\n", ($Slic3r::Config->gcode_comments ? ' ; use absolute distances for extrusion' : '');
				}
			}
		}
	}
	...
	if ($Slic3r::Config->end_gcode ne "") {
		#Print out the end code
