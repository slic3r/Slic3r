...
sub write_gcode {
    ...
    # write the comments to the beginning of the gcode after everything has already been figured out (i.e. total_extrusion_length etc)
    {
		local @ARGV = ($file);
		local $^I = '.bac';
		while(<>){
			if ($. == 1) {
				printf "; Generated by Slic3r $Slic3r::VERSION on %04d-%02d-%02d at %02d:%02d:%02d\n\n",
        		$lt[5] + 1900, $lt[4]+1, $lt[3], $lt[2], $lt[1], $lt[0];
				
				print "; $_\n" foreach split /\R/, $self->expanded_output_filepath($Slic3r::notes);
				print "\n" if $Slic3r::notes;
				print;
			}else{
				print;
...
sub total_extrusion_volume {
    my $self = shift;
    if ($self->total_extrusion_length){
...
sub expanded_output_filepath {
    ...
    my $extras = {
        ...
        extrusion_width_ratio			=> $Slic3r::flow->width / Slic3r::Config->get('layer_height'),
        total_extrusion_length			=> $self->total_extrusion_length,
        total_extrusion_volume			=> $self->total_extrusion_volume || undef,
        single_wall_width				=> $Slic3r::flow->width,
    };
    
    if ($Slic3r::first_layer_flow){
    	$extras->{single_wall_width_first_layer} = $Slic3r::first_layer_flow->width;
    }else{
    	$extras->{single_wall_width_first_layer} = $Slic3r::flow->width;
    }
    
    return Slic3r::Config->replace_options($path, $extras);
}
