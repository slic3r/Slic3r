...
sub make_perimeters {
    ...
    foreach my $surface (@surfaces) {
        ...
        my @intervals = $ma->offset_interval_filter(
          2    * $self->perimeter_flow->scaled_spacing, # tool diam, or normal offset (used after initial offset)
        ...
        for (my $loop = 0; $loop <= $loop_number; $loop++) {
            ...
            foreach my $expolygon (@last_offsets) {
                ...
                my @offsets = Slic3r::Geometry::Clipper::ex_int_offset2($expolygon, -(1 + ($loop == 0 ? 0:0.5))*$spacing,  +0.5*$spacing);
